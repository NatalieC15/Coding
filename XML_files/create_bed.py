#Importing the ElementTree (import should always be at the top of an python code)
...
#There are also two functions which have a test in a separate file see the GitHub repository for more details.
...
while True:
	...
	#The alternative output for the second test code for when the file has not been saved correctly:
		...
		print('Error, check the correct LRG XML file has been downloaded from https://www.lrg-sequence.org/'
			  ', saved into the right folder (i.e. XML_files) with the correct format (e.g. LRG_1.xml)')
		break
	...
	#Finding the chromosome number, start and end coordinates within the XML file using the second function, this finds the build number GRCh37.p13 for LRG_1.xml file but may not converse for every file.
